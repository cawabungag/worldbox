...
namespace XFlow.EcsLite {
    ...
    public static class Extensions {
        ...
        [Obsolete("replace with DeleteEntityHere")]
...
#endif
            return systems.Add (new DeleteEntityHereSystem<T> (systems.GetWorld (worldName)));
        }
        
        public static EcsSystems DeleteEntityHere<T> (this EcsSystems systems, string worldName = null) where T : struct {
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
            if (systems.GetWorld (worldName) == null) { throw new System.Exception ($"Requested world \"{(string.IsNullOrEmpty (worldName) ? "[default]" : worldName)}\" not found."); }
#endif
            return systems.Add (new DeleteEntityHereSystem<T> (systems.GetWorld (worldName)));
        ...
        public static EcsSystems DeleteComponentHere<T> (this EcsSystems systems, string worldName = null) where T : struct {
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
            if (systems.GetWorld (worldName) == null) { throw new System.Exception ($"Requested world \"{(string.IsNullOrEmpty (worldName) ? "[default]" : worldName)}\" not found."); }
#endif
            return systems.Add (new DeleteComponentHereSystem<T> (systems.GetWorld (worldName)));
        }
    }
    ...
    [Il2CppSetOption (Option.ArrayBoundsChecks, false)]
#endif
    public sealed class DeleteEntityHereSystem<T> : IEcsRunSystem where T : struct {
        readonly EcsFilter _filter;
        readonly EcsPool<T> _pool;
        readonly EcsWorld _world;

        public DeleteEntityHereSystem (EcsWorld world) {
            _filter = world.Filter<T> ().End ();
            _world = world;
            if (_world.GetDebugName() == "main")
                throw new Exception("DeleteEntityHereSystem should not be used with main world");
        }
    ...
    [Il2CppSetOption (Option.ArrayBoundsChecks, false)]
#endif
    public sealed class DeleteComponentHereSystem<T> : IEcsRunSystem where T : struct {
        readonly EcsFilter _filter;
        readonly EcsPool<T> _pool;
        readonly EcsWorld _world;

        public DeleteComponentHereSystem (EcsWorld world) {
            _filter = world.Filter<T> ().End ();
            _world = world;
        }
    ...
    public sealed class EcsGroupSystem :
        ...
        public void Run (EcsSystems systems) {
            foreach (var entity in _filter) {
                ref var evt = ref _pool.GetRef (entity);
                if (evt.Name == _name) {
                    _state = evt.State;
                    _pool.Del (entity);
                }
        ...
        {
            for (var i = _allSystems.Length - 1; i >= 0; i--) {
                if (_allSystems[i] is IEcsWorldChangedSystem worldChangedSystem) {
                    worldChangedSystem.WorldChanged(world);
                }
            }
        }
    }
