...
namespace XFlow.EcsLite {
    ...
    public sealed class EcsFilter {
        ...
        public int GetAt (int index) {
            ...
            if (_entitiesCount <= index) { throw new Exception ("_entitiesCount <= index"); }
#endif
        ...
        [MethodImpl (MethodImplOptions.AggressiveInlining)]
        internal void InternalResetAndResize (int newSize)
        {
            _entitiesCount = 0;
            Array.Clear(SparseEntities, 0, SparseEntities.Length);
            ResizeSparseIndex(newSize);
        }
        ...
        {
            var memorySize = 0;
            var denseEntitiesMemorySize = Utils.Utils.GetArrayMemorySize(_denseEntities);
            var sparseEntitiesMemorySize = Utils.Utils.GetArrayMemorySize(SparseEntities);
            var delayerOpMemorySize = Utils.Utils.GetArrayMemorySize(_delayedOps);
            
            memorySize += denseEntitiesMemorySize + sparseEntitiesMemorySize + delayerOpMemorySize;
